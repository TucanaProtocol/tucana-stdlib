module tucana_std::skip_list_u64 {
    use std::vector::{Self, push_back};
    use initia_std::table;
    use tucana_std::option_u64::{Self, OptionU64, none, some, is_none, is_some, swap_or_fill, is_some_and_lte};
    use tucana_std::random::{Self, Random};
    ...
    /// The skip list.
    struct SkipListU64<V: store> has key, store {
    ...
    public fun new<V: store>(max_level: u64, list_p: u64, seed: u64): SkipListU64<V> {
        let list = SkipListU64<V> {
            head: vector::empty(),
            tail: none(),
            level: 0,
            max_level,
    ...
    public fun length<V: store>(list: &SkipListU64<V>): u64 {
        table::length(&list.inner)
    }

    /// Returns true if the skip list is empty (if `length` returns `0`)
    public fun is_empty<V: store>(list: &SkipListU64<V>): bool {
        length(list) == 0
    }

    /// Return the head of the skip list.
    public fun head<V: store>(list: &SkipListU64<V>): OptionU64 {
        if (is_empty(list)) {
            return none()
        };
    ...
    public fun tail<V: store>(list: &SkipListU64<V>): OptionU64 {
        list.tail
    }

    /// Destroys an empty skip list
    /// Aborts with `ETableNotEmpty` if the list still contains values
    public fun destroy_empty<V: store + drop>(list: SkipListU64<V>) {
        assert!(length(&list) == 0, ESkipListNotEmpty);
        let SkipListU64<V> {
            head: _,
            tail: _,
            level: _,
    ...
    public fun contains<V: store>(list: &SkipListU64<V>, score: u64): bool {
    ...
    public fun borrow<V: store>(list: &SkipListU64<V>, score: u64): &V {
    ...
    public fun borrow_mut<V: store>(list: &mut SkipListU64<V>, score: u64): &mut V {
    ...
    public fun borrow_node<V: store>(list: &SkipListU64<V>, score: u64): &Node<V> {
    ...
    public fun borrow_mut_node<V: store>(list: &mut SkipListU64<V>, score: u64): &mut Node<V> {
        table::borrow_mut<u64, Node<V>>(&mut list.inner, score)
    }

    /// Return the metadata info of skip list.
    public fun metadata<V: store>(list: &SkipListU64<V>): (vector<OptionU64>, OptionU64, u64, u64, u64, u64) {
    ...
    public fun insert<V: store>(list: &mut SkipListU64<V>, score: u64, v: V) {
        ...
        while(l > 0) {
            ...
            while (is_some_and_lte(opt_next_score, score)) {
                let node =
                    table::borrow_mut<u64, Node<V>>(&mut list.inner, option_u64::borrow(opt_next_score));
    ...
    public fun remove<V: store>(list: &mut SkipListU64<V>, score: u64): V {
        assert!(contains(list, score), ENodeDoesNotExist);
        let (l, nexts) = (list.level, &mut list.head);
        let node: Node<V> = table::remove(&mut list.inner, score);
        while (l > 0) {
            let opt_next_score = vector::borrow_mut(nexts, l - 1);
            while (is_some_and_lte(opt_next_score, score)) {
    ...
    public fun find_next<V: store>(list: &SkipListU64<V>, score: u64, include: bool): OptionU64 {
    ...
    public fun find_prev<V: store>(list: &SkipListU64<V>, score: u64, include: bool): OptionU64 {
        let opt_finded_score = find(list, score);
        if (is_none(&opt_finded_score)) {
            return opt_finded_score
        };
    ...
    fun find<V: store>(list: &SkipListU64<V>, score: u64): OptionU64 {
        if (length(list) == 0) {
            return none()
        };
    ...
    fun rand_level<V: store>(seed: u64, list: &SkipListU64<V>): u64 {
        let level = 1;
        let mod = list.list_p;
        while ((seed % mod) == 0) {
            mod = mod * list.list_p;
    ...
    fun create_node<V: store>(list: &mut SkipListU64<V>, score: u64, value: V): (u64, Node<V>) {
    ...
    #[test_only]
    fun print_skip_list<V: store>(list: &SkipListU64<V>) {
        debug::print(list);
        if (length(list) == 0) {
            return
        };
    ...
    #[test_only]
    fun check_skip_list<V: store>(list: &SkipListU64<V>) {
        if (list.level == 0) {
    ...
    #[test_only]
    fun get_all_socres<V: store>(list: &SkipListU64<V>): vector<u64> {
    ...
    fun test_new(
        ...
        let skip_list = new<u256>(16, 2, 12345);
        check_skip_list(&skip_list);
        move_to(&owner, skip_list)
    }
    ...
    #[test_only]
    fun add_node_for_test<V: store + copy + drop>(list: &mut SkipListU64<V>, size: u64, seed: u64, value: V) {
    ...
    fun new_list_for_test<V: store + copy + drop>(
        max_leveL: u64, list_p: u64, size: u64, seed: u64, value: V
    ): SkipListU64<V> {
        let list = new<V>(max_leveL, list_p, seed);
    ...
    fun test_insert(
        ...
        let list = new_list_for_test<u256>(16, 2, 3000, 1234, 0);
        move_to(&owner, list)
    }
    ...
    fun test_find(
        ...
        let list = new_list_for_test<u256>(16, 2, 1000, 12345, 0);
    ...
    fun test_remove(
        ...
        let list = new_list_for_test<u256>(16, 2, 1000, 5678, 0);
        let scores = get_all_socres(&list);
